...
public class JDBCTest01 {
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException {
		...
		//				  jdbc:oracle:thin:@multi0729_high?TNS_ADMIN=C:\\\\DataBaseTest\\\\Wallet_Multi0729
		...
		//3. 결과처리
		//next() : 커서를 다음 row로 이동시킨 후 데이터가 있는지 없는지 판단(있으면 true, 없으면 false)
		while(rs.next()) {
			System.out.println(rs.getString(1)+" "+rs.getString(2)+" "+rs.getInt("SALARY"));
									  // getXxxx() : 현재 커서거 가리키는 row의 특정 컬럼값을 가지고 오겠다.
		}
		//4. 종료
		rs.close();
		stmt.close();
