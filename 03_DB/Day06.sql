...
VALUES(500,'이창진','800101-1234567','leecj@or.kr','01011112222','D1','J7','S4',3100000,0.1,'200',
...
VALUES(900,'김창진','660101-2345678','kimcj@or.kr','01033334444','D1','J7','S3','4300000',0.2,'200',
	   CURRENT_DATE,NULL,DEFAULT);
...
-- UPDATE 테이블명 SET 컬럼명=수정할값, 컬럼명=수정할값,...
-- [WHERE 조건]
...
-- '621230','631126','850705'로 변경하는 UPDATE구문 3개 작성
...
-- 정중하, 전형돈 사원의 급여, 보너스도 유재식 사원과 동일하게
...
-- UPDATE 시에 변경할 값이 해당 컬럼의 제약조건을 위배하지 않게 변경해야 한다.
...
DELETE FROM EMP_SALARY WHERE BONUS != 0.25;
...
-- DEPT_ID 컬럼의 크기를 3으로 변경 후에는 INSERT 잘된다.
INSERT INTO DEPT_COPY2 VALUES('D10', '샘플부서', 'L1', '한국');
...
-- COMMIT : 트랜잭션이 종료될 떄 정상적으로 종료되었다면 변경한 사항을 영구히 DB에 저장.
...
-- ----------------------------------------
-- DDL 명령어는 트랜잭션의 개념이 아니다.
-- COMMIT/ROLLBACK 불가능
-- 또한 이전 DML(INSERT, UPDATE, ...)을 실행 후
-- DDL 명령어 사용하는 이전까지의 데이터 조작 내용이 자동 COMMIT 처리된다.
 ...
 * 
 * 관한 뺏기도 가능. 실행 ㄴㄴ
 * 	REVOKE CREATE VIEW FROM MULTI;
 */

-- VIEW : SELECT를 실행한 결과 화면을 담는 객체.
-- 		  VIEW에는 SELECT 문장 자체를 저장. 호출할 때마다 해당 쿼리를 실행해서 결과를 보여준다.

-- 실질적으로 데이터를 담고 있지 않다.
-- 사용자에게 노출하고 싶지 않은 정보나
-- 업무에 필요한 정보만 조회하고자 할 때 사용.
...
-- 사번, 이름, 직급명, 부서명, 근무지역명을 조회
-- 그 결과를 V_RES_EMP 라는 뷰를 만들어 뷰를 통해 결과를 조회.
-- 1) 서브쿼리 준비
