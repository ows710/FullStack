...
-- EMPLOYEE의 DEPT_CODE와 DEPARTMENT의 DEPT_ID가 같은 것끼리 합친다.



SELECT EMPLOYEE.EMP_ID, EMP_NAME, JOB_CODE, DEPT_TITLE, LOCATION_ID
FROM EMPLOYEE
-- JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
FROM EMPLOYEE
JOIN JOB ON(EMPLOYEE.DEPT_CODE = JOB.JOB_CODE);
...
-- 테이블을 JOIN 통해 하나로 합칠 때
-- INNER JOIN은 둘 모두 일치하는 데이터만 합치고,
-- OUTER JOIN은 둘 중 하나, 혹은 둘 모두가 가진 데이터들을 합친다.


-- INNER JOIN
-- JOIN을 했을 때 EMPLOYEE 테이블 DEPT_CODE 컬럼에 NULL인 데이터는 사라진다.
...
-- 한극(KO)과 일본(JP)에 근무하는 직원들의 정보 조회
-- 사원명(EMPLOYEE), 부서명(DEPARTMENT), 지역명(LOCATION), 국가명(NATIONAL)
-- NATIONAL 테이블 활용
...
-- RANK() : 동일한 순번이 있을 경우 이후의 순번은 이전 동일한 순번의 개수만큼
--          건너뛰고 순번을 매긴다.
--1
--2
--3
...
-- DENS_RANK() : 동일한 순번이 있을 경우 이후 순번에 영향을 미치지 않는다.
--1
--2
--3
--4
...
-- 상호 연관 쿼리 : 상관쿼리
-- 일반적으로 서브쿼리 서브쿼리대로, 메인쿼리 서브쿼리와의 결과만을 받아서 실행.
-- 메인 쿼리가 사용하는 컬럼값, 계산 등을 서브쿼리에 적용하여 서브쿼리 실행 시
...
-- 이름(EMP_NAME), 직급명(JON_NAME), 급여정보(SALARY) 조회

SELECT EMP_NAME, JON_NAME, SALARY
FROM EMPLOYEE E
WHERE SALARY > (
				SELECT AVG(SALARY)
				FROM EMPLOYEE E2
 ...
 * 				 테이블 내에서 단 1개만 존재.
 *  FOREIGN KEY - 다른 테이블에서 저장된 값을 연결지어서 참조로 가져오는 데이터에 지정하는 제약조건.
 */
