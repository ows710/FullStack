...
-- NAME은 NOT NULL 제약조건을 등록하지 않았기에 NULL 값이 잘 들어간다.
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(MULTI.SYS_C007492)에 위배됩니다
-- ID 값이 중복으로 INSERT 되려 하였기에 제약조건 위반
INSERT INTO USER_UNIQUE
VALUES(1, 'USER01', 'PASS01', '이창진');
...
JOIN USER_CONS_COLUMNS C2 ON (C1.CONSTRAINT_NAME = C2.CONSTRANINT_NAME)
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(MULTI.SYS_C007493)에 위배됩니다
INSERT INTO USER_UNIQUE2 VALUES(1, 'USER01', 'PASS01', '이창진');
...
INSERT INTO USER_UNIQUE3 VALUES(1, 'USER02', 'PASS02', '김창진');
...
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(MULTI.SYS_C007498)이 위배되었습니다
...
-- PRIMARY KEY 제약조건 --
-- '기본키 제약조건'
-- 테이블 내의 데이터들 중에서 한 행을 식별하기 위한 고유값
-- 테이블 전체에 대한 각 데이터(ROW)의 식별자 역할을 수행시키는 제약조건.
-- NOT NULL과 UNIQUE 제약 조건이 부여.
-- 값이 반드시 들어가고, 중복이 있어서도 안된다.

-- 테이블마다 기본키는 하나만 존재.(하나의 컬럼에만 기본키 제약조건을 설정 가능)
-- PRIMARY KEY는 한 컬럼에 적용가능하고. 여러 컬럼을 묶어서도 적용 가능.
...
SELECT * FROM USER_PK_TABLE;
-- 기본키 제약조건을 설정하면 NOT NULL과 UNIQUE가 함께 설정된다.
...
-- 컬럼의 크기는 편하신대로, 테이블 생성 후에는 INSERT 5번 이상 진행 후
-- SELECT까지 확인!
...
CREATE TABLE MEMBER(
	...
	M_NAME VARCHAR2(15),
	M_GENDER CHAR(1) CHECK(M_GENDER IN('F', 'M')),
	M_PHONE VARCHAR(20)
);

SELECT * FROM MEMBER;

INSERT INTO MEMBER VALUES(1, 'USER01', 'PASS01', '홍길동', 'F', '010-1111-2222');
INSERT INTO MEMBER VALUES(2, 'USER02', 'PASS02', '김길동', 'M', '010-2222-2222');
INSERT INTO MEMBER VALUES(3, 'USER03', 'PASS03', '박길동', 'M', '010-3333-2222');
INSERT INTO MEMBER VALUES(4, 'USER04', 'PASS04', '최길동', 'F', '010-4444-2222');
INSERT INTO MEMBER VALUES(5, 'USER05', 'PASS05', '정길동', 'F', '010-5555-2222');
...
-- 참조하는 테이블 값에 접근
-- FOREIGN KEY 제약조건을 통해 테이블들 간에 관계(RELATIONSHIP) 형성된다.
...
-- 참조하고 있는 원본 테이블의(부모) 컬럼 값이 삭제될 때
-- 참조한 값을 어떻게 처리할 것인지 설정하는 옵션

-- 일반적으로 참조하는 원본 컬럼의 내용을 삭제하려 한다면
-- 외래키로 사용 중인 자식 컬럼이 존재할 때는 함부로 삭제 불가능하다. (SQL Error [2292] [23000]: ORA-02292)
...
-- SUB QUERY를 활용하여 테이블 만들기
-- 컬럼명, 데이터 타입, 값, NOT NULL 복사 가능.
...
-- 제약조건을 중간에 변경할 경우 이미 저장한 값 등을 파악하여 제약조건에 위배되지 않는지 확인해야 한다.
...
ON(C1.TABLE_NAME = C2.TABLE_NAME AND C1.CONSTRAINT_NAME = C2.CONSTRAINT_NAME)
