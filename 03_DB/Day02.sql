...
-- 선택하여 조회하는 조건 절
-- 조건은 여러 개의 조건도 가능 -> AND, OR 명령어와 함께 사용

-- 부서 코드(DEPT_CODE) 'D6'인 사원의 정보를 조회.
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6';

-- 직급(JOB_CODE)이 'J1'인 사원의 
-- 사번(EMP_ID), 사원명(EMP_NAME), 직급코드(JOB_CODE), 부서코드(DEPT_CODE) 조회
...
-- 사원명, 이메일, 급여, 입사일자를 조회
-- 단, 조회하는 컬럼에 별칭 부여
SELECT EMP_NAME AS "사원명", EMAIL AS 이메일, SALARY "급여", HIRE_DATE 입사일자
FROM EMPLOYEE
WHERE EMP_ID = 205;
...
-- 연결 연산자
-- '||'
-- 여러 컬럼의 결과나 값을 하나의 컬럼으로 묶을 때 사용하는 연산자
...
-- EMPLOYEE에서
-- 사원 이름에 가운데가 '중' 들어가는 사원 정보 조회.
SELECT *
FROM EMPLOYEE
WHERE EMP_NAME LIKE '_중_';

-- EMPLOYEE에서
-- 주민등록번호(EMP_NO) 기준 여성인 사원의 정보 조회
SELECT *
FROM EMPLOYEE
...
-- DUAL : 임시테이블(가상의 테이블)
-- 		  SELECT 문에서 리터럴을 활용한 테스트를 진행하고자 할 때 사용하는 임시 테이블
