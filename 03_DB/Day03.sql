...
-- 각 행마다 반복적으로 적용되고 실행되는 함수를 단일 행 함수(SINGLE ROW FUNCTION)
...
SELECT AVG(SALARY) "평균 급여", MAX(SALARY) 높은급여, MIN(SALARY), SUM(SALARY)
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
FROM EMPLOYEE;
-- COUNT는 컬럼 안에 NULL 값이 존재할 경우 이를 카운팅하지 않는다.
...
-- ORDER BY 절
-- SELECT를 통해 조회한 행의 결과들을 특정 기준에 맞춰
-- 정렬하는 구문.
-- ASC : 오름차순, DESC : 내림차순
...
-- ORDER BY 3 DESC; -- 해당 번째 선택해서 사용 가능
...
-- 한 테이블 내에서 소그룹 별로 조회하고 할 때 선언하는 구문
...
-- GROUP BY에서 주어진 컬럼뿐만이 아니라 함수식도 사용 가능
...
--
-- SET OPERATOR --
-- 두 개 이상의 SELECT한 결과를 합치거나, 중복 제거하거나 하는
-- 집합 형태의 결과로 조회하는 명령어
...
SELECT EMP_NAME 직원명, JOB_CODE 직급코드, TO_CHAR((SALARY + (SALARY*BONUS))*12, 'L999,999,999') "연봉(원)"
FROM EMPLOYEE;

--3. 부서코드가 D5, D9인 직원들 중에서 2004년도에 입사한 직원의 
--   수 조회함.
--   사번 사원명 부서코드 입사일
SELECT COUNT(EMP_ID) 직원수
FROM EMPLOYEE
...
--   단, 주말도 포함함
SELECT EMP_NAME 직원명, HIRE_DATE 입사일, LAST_DAY(HIRE_DATE) - HIRE_DATE + 1 근무일수
FROM EMPLOYEE;
...
SELECT EMP_NAME 직원명, DEPT_CODE 부서코드,
	   ...
	   FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6), 'YYMMDD')) / 12) "나이(만)"
FROM EMPLOYEE;
...
SELECT 
    COUNT(*) 전체직원수,
    COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2001', 1, NULL)) "2001년",
    COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2002', 1, NULL)) "2002년",
    COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2003', 1, NULL)) "2003년",
    COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2004', 1, NULL)) "2004년"
FROM EMPLOYEE;
...
SELECT EMP_ID 사번, EMP_NAME 사원명, JOB_CODE 직급코드,
	   DECODE(JOB_CODE, 'J5', '20%', 'J6', '15%', 'J7', '10%', '5%') "인상될 급여"
FROM EMPLOYEE;
...
--  조회 하시오.)
SELECT JOB_CODE "직급 코드", SUM(SALARY) "급여 합계", AVG(SALARY) "급여 평균", COUNT(JOB_CODE) "인원 수"
FROM EMPLOYEE
GROUP BY JOB_CODE
